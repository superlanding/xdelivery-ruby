...
describe 'Xdelivery::API::Response::Orders' do
  ...
  describe "當 merchant_no, access_key 登入情況" do
    ...
    describe "當回應帶著 history_id 時 ..." do
      before do
        @data = "{\"status\":true,\"history_id\":5566,\"orders\":[{\"ship_no\":\"F00000001111\",\"order_id\":\"SP19049\"}]}"
        stub_request(:any, Xdelivery::API::Base::PRODUCTION_BASE_URL).to_return(body: @data)
        @http_response = RestClient.post(Xdelivery::API::Base::PRODUCTION_BASE_URL, {})
      ...
      it "#history_id" do
        assert_equal 5566, @response.history_id
      end
    end

    describe "當回應沒有 history_id 時 ..." do
      before do
        @data = "{\"status\":false,\"orders\":[]}"
        stub_request(:any, Xdelivery::API::Base::PRODUCTION_BASE_URL).to_return(body: @data)
        @http_response = RestClient.post(Xdelivery::API::Base::PRODUCTION_BASE_URL, {})
      ...
      it "#history_id == nil" do
        assert_nil @response.history_id
      end
    end
