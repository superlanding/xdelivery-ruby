...
describe 'Xdelivery::Client (真實測試)' do
  ...
  it "#get_shops!" do
    with_development! do
      @response = @client.get_shops!
      assert_equal true, @response.status?
      assert_equal true, @response.auth?
      assert_equal 2, @response.shops.count
      assert_equal 1, @response.shops[0].id
      assert_equal '李焰的敗家日記', @response.shops[0].title
      assert_equal true, @response.shops[0].default_shop
    end
  end
    ...
    it "OK 拋送訂單 (發票: Email) (指定 invoice setting)" do
      with_development! do
        @response = @client.create_orders! do |orders|
          ...
          orders.add(@order_params.merge(shop_id: 2), invoice_params)
        end
      end
      assert_equal true, @response.status?
      assert_equal "SP19049", @response.orders[0].order_id
    ...
    it "FAILD 拋送訂單 (發票: Email 未設定)" do
      with_development! do
        @response = @client.create_orders! do |orders|
          ...
          orders.add(@order_params.merge(shop_id: 3), invoice_params)
        end
      end
      ...
      assert_equal '沒有設定商店', @response.orders[0].errors['shop_id']
    end
